...
<title>หาเพื่อน หาแฟน หากิ๊ก คุยเอ็ม Msn Facebook Twister BB ชุมชนออนไลน์ | <?=$titler[1];?></title>
<META NAME="keywords" CONTENT="หาเพื่อน, หาแฟน, หากิ๊ก, Msn, MsnList, Facebook, Twister, BB, คุยเอ็ม, แชท"> 
<META NAME="description" CONTENT="<?=$titler[1];?> หาเพื่อน หาแฟน คุย msn หาเพื่อน msn Facebook Twister BB เล่นกล้อง โชว์กล้อง หล่อ สวย นับแสนคน มีให้คุณเลือกอย่างจุใจ คลิ๊กเข้ามาเลย!">
              ...
              <table width="665" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td height="50" background="img/bg-msn-list.png"><table width="610" border="0" align="right" cellpadding="0" cellspacing="0">
                      <tr>
                        <td height="30" align="left"><font size="4" color="#333333"><strong>
                          <? if(isset($_GET[zone])){ ?>
                          หาเพื่อน Social Network
                          <?=$zone_name;?>
                          100 คนล่าสุด
                          <? }else{ ?>
                          หาเพื่อน Social Network ทั่วทุกภาค 100 คนล่าสุด
                          <? } ?>
                        </strong></font></td>
                      </tr>
                      <tr>
...
$zone=$_GET[zone];
if(isset($_GET[zone])){
$s="SELECT post_friend.*, post_friend_option.*, province.PROVINCE_NAME, talk_with.talk_with, talk_topic.talk_topic FROM post_friend ";
$s.="INNER JOIN post_friend_option ON post_friend.id=post_friend_option.post_id ";
$s.="INNER JOIN province ON post_friend.province_id=province.PROVINCE_ID ";
$s.="INNER JOIN talk_with ON post_friend.talk_with=talk_with.id ";
$s.="INNER JOIN talk_topic ON post_friend.talk_topic=talk_topic.id ";
$s.="WHERE post_friend.active=1 AND province.GEO_ID='$zone' ORDER BY post_friend.id DESC LIMIT 0,100 ";
}else{
$s="SELECT post_friend.*, post_friend_option.*, province.PROVINCE_NAME, talk_with.talk_with, talk_topic.talk_topic FROM post_friend ";
$s.="INNER JOIN post_friend_option ON post_friend.id=post_friend_option.post_id ";
$s.="INNER JOIN province ON post_friend.province_id=province.PROVINCE_ID ";
$s.="INNER JOIN talk_with ON post_friend.talk_with=talk_with.id ";
$s.="INNER JOIN talk_topic ON post_friend.talk_topic=talk_topic.id ";
$s.="WHERE post_friend.active=1 ORDER BY post_friend.id DESC LIMIT 0,100 ";
}
$sql=$s;
